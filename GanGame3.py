...
class GANGame:
    def __init__(self, generador, discriminador, device):
        ...
        self.criterio_combinado = CombinedLoss(device=self.device)

        # Criterio para calcular la pérdida
        self.criterio = nn.BCELoss()
        
        
    def pre_entrenar_discriminador(self, cargador_datos_preentrenamiento, num_epochs_preentrenamiento, umbral=0.5, num_imagenes_visualizar=5):
        for epoch in range(num_epochs_preentrenamiento):
            perdida_d_epoch = 0
            contador_imagenes = 0
            for imagenes_buenas, imagenes_rayadas in cargador_datos_preentrenamiento:
                imagenes_buenas, imagenes_rayadas = imagenes_buenas.to(self.device), imagenes_rayadas.to(self.device)

                # Entrenar con imágenes buenas (reales)
                self.discriminador.zero_grad()
                etiquetas_reales = torch.ones(imagenes_buenas.size(0), device=self.device)
                salida_reales = self.discriminador(imagenes_buenas).view(-1)
                perdida_d_real = self.criterio(salida_reales, etiquetas_reales)

                # Entrenar con imágenes rayadas (como si fueran generadas)
                etiquetas_falsas = torch.zeros(imagenes_rayadas.size(0), device=self.device)
                imagenes_falsas = self.generador(imagenes_rayadas).detach()
                salida_falsas = self.discriminador(imagenes_falsas).view(-1)
                ...
                perdida_d_epoch += perdida_d.item()

                # Visualizar un número limitado de imágenes y su etiqueta predicha
                for i in range(imagenes_buenas.size(0)):
                    if contador_imagenes < num_imagenes_visualizar:
                        imagen_ejemplo = imagenes_buenas[i].to('cpu')
                        etiqueta_predicha = 'Real' if salida_reales[i].item() > umbral else 'Falsa'
                        self.visualizar_imagen_con_etiqueta(imagen_ejemplo, etiqueta_predicha)
                        contador_imagenes += 1
                    else:
                        break
            ...
            print(f'Pre-Entrenamiento - Epoch [{epoch+1}/{num_epochs_preentrenamiento}], Pérdida D: {perdida_d_epoch}')


    def visualizar_imagen_con_etiqueta(self, imagen, etiqueta):
        plt.figure()
    ...
    def entrenar_lote(self, imagenes_rayadas, imagenes_buenas):
        ...
        salida_discriminador = self.discriminador(imagenes_generadas)
        salida_discriminador = salida_discriminador.view(salida_discriminador.size(0), -1).mean(1)

        # Calcular la pérdida del generador
        perdida_g = self.criterio_combinado(imagenes_generadas, imagenes_buenas, salida_discriminador)  # Aquí usamos la CombinedLoss
    ...
    def entrenar(self, cargador_datos, num_epochs, intervalo_visualizacion=5, intervalo_guardado=10, ruta_guardado="/home/rodrigo/Documentos/5to semestre/machin/modelosGAN/gan", num_epochs_preentrenamiento=25):
        
        self.pre_entrenar_discriminador(cargador_datos, num_epochs_preentrenamiento)
        
        for epoch in range(num_epochs):
            self.criterio_combinado.ajustar_lambdas(epoch, num_epochs)  # Ajuste dinámico de lambdas
...
class CombinedLoss(nn.Module):
    def __init__(self, lambda_adv=0.15, lambda_rec=0.9, lamda_estrc=0.1, device=None):
        super(CombinedLoss, self).__init__()
        self.alpha = lambda_adv
        self.beta = lambda_rec
        self.gamma = lamda_estrc
        self.adversarial_loss = nn.BCEWithLogitsLoss()
        self.reconstruction_loss = nn.L1Loss()
        self.ssim_loss = SSIM(window_size=11, size_average=True)

    def forward(self, real_images, generated_images, discriminator_outputs):
        # Pérdida adversaria
        adversarial_loss = self.adversarial_loss(discriminator_outputs, torch.ones_like(discriminator_outputs))

        # Pérdida de reconstrucción
        reconstruction_loss = self.reconstruction_loss(generated_images, real_images)

        # Pérdida SSIM
        ssim_loss = 1 - self.ssim_loss(generated_images, real_images)

        # Combinar pérdidas
        total_loss = self.alpha * adversarial_loss + self.beta * reconstruction_loss + self.gamma * ssim_loss
        return total_loss
